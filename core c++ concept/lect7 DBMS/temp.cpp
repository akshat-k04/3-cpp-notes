...
int main(){
    ...
    for(int r=0 ;r<t ;r++){
        ...
        vii for_d(n) , back_d(n) ;
        for(int e=0 ;e<n-1 ;e++)for_d[e] = a[e] -a[e+1] ;
        for(int e=1  ;e<n ;e++)back_d[e] = a[e]-a[e-1] ;

        multiset<<pair<int,pair<int,int>>> temper ; // diff , 0/1 , index
        for(int i =0 ;i<n-1 ; i++)temper.insert({for_d[i],{0,i}}) ;
        for(int i =1 ; i<n ;i++)temper.insert({back_d[i],{1,i}}) ;

        ll ans =0 ;
        for(int e=0 ;e<n ;e++)ans+= a[e] ;

        int count=0 ;
        while(count<k){
            count++ ;
            pair<int, pair<int,int>> info = *(-- temper.end()) ; // diff , 0/1 , index

            if(info.second.first==0 && info.second.second==n-1){
                for_d[n-2] = 0 ; back_d[n-1] =0 ;
                temper.erase((--temper.end())) ;
                temper.insert({0,{0,n-1}}) ;
            }
            else if (info.second.first ==1  && info.second.second==0){
                for_d[0] = 0 ; back_d[1] = 0 ;
                temper.erase((--temper.end())) ;
                temper.insert({0,{1,0}}) ;
            }
            else {
                if(info.second.first==0){
                    for_d[]
                }
                else{
