...
int main(){
    ...
    for(int r=0 ;r<t ;r++){
        ...
        cin>>q ;
        vpii lr(q) ;
        for(int e=0 ;e<q ;e++)cin>>lr[e].first >> lr[e].sec ;

        string final_s =s ,final_T = t ;
        for(int e=0 ;e<n-2 ;e++)if(s[e]==s[e+2] && s[e]=='0')final_T[e+1]='1' ;
        for(int e=0 ;e<n-2 ;e++)if(final_T[e]==final_T[e+2] && final_T[e]=='1')final_s[e+1]='1' ;

        vii prefix_sum(n+1) ;
        for(int e=0 ;e<n ;e++)prefix_sum[e+1] = prefix_sum[e]+ (final_s[e]=='1') ;

        for(int e=0 ;e<q ;e++){
            // check final_T[r-2]with T[r] if not equal if T[r]!=final_T[r] then reduce ans by 1
            
            
            lr[e].first-- ; lr[e].second-- ;
            int l = lr[e].first , r = lr[e].second ;
            int ans = prefix_sum[r+1]-prefix_sum[l] ;
            // cout<<ans<< " ";
            if(r-l+1>3){
                if(final_T[r-2]!= t[r]){
                    if(final_T[r-2]=='1' && final_s[r-1]!=s[r-1])ans-- ;
                }
                if(final_T[l+2]!=t[l]){
                    if(final_T[l+2]=='1' && final_s[l+1]!= s[l+1])ans-- ;
                }
                if(final_s[l]!=s[l])ans-- ;
                if(final_s[r]!=s[r])ans-- ;
            }
            else if(r-l+1==3){
                ans= (s[l]=='1') ; ans+= (s[l+2]=='1') ;
                if(t[l]==t[r] && t[l]=='1')ans+=1 ;
                else ans+= (s[l+1]=='1') ;
            }
            else{
                ans = 0 ;
                for(int f=l ;f<=r ;f++)ans+= (s[f]=='1') ;
            }
            cout<<ans<<endl ;
        }
    }
