...
int main(){
    ...
    for(int r=0 ;r<t ;r++){
        ...
        cin>> s ;
                ...
                vii inter ;
                for(int e=0 ;e<n ;e++)inter.pb(s[e]-'0') ;
                int real_ans = primeInt ;
                for(int e=0 ;e<n-1 ;e++){
                    vii personalized ;
                    for(int f = 0 ; f<n ;f++){
                        if(f==e){
                            personalized.pb(10*inter[e] + inter[e+1]) ;
                            f++ ; continue ;
                        }
                        personalized.pb(inter[f]) ;
                    }
                    int temp_ans =personalized[0] ;
                    for(int f=1 ;f<personalized.size() ;f++){
                        if(temp_ans==0||personalized[f]==0)temp_ans=0 ;
                        else{
                            if(temp_ans==1)temp_ans = personalized[f] ;
                            else if(personalized[f]!=1)temp_ans+= personalized[f] ;
                        }
