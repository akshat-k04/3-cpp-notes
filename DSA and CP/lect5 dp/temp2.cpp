...
int solve(vii &a ,vii &ans,int indx,int ini,int end){
    int len = end-ini+1 ;
    if(len==2){
        ans[indx]=max(a[ini],a[end]) ;
        return max(a[ini],a[end]) ;
    }
    else{
        ans[indx]= max(a[ini]+solve(a,))
    }
}
...
int main(){
    ...
    vii ans(n/2,-1) ;
    cout<<solve(a,ans,(n/2)-1,0,n-1)<<endl ;
