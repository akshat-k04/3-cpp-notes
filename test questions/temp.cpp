...
int dfs(vector<vector<int>> & list ,int indx , vector<int> &nums,int parent){
    int ans =1 ;
    for(auto child: list[indx]){
        if(child== parent) continue ;
        ans += dfs(list,child,nums,indx) ;
    }
    nums[indx]= ans ;
    return ans ;
}

ll minidiff(vii & fac,int maxsum){
    int n = fac.size() ;
    vector<vii> dp(n+1, vii (maxsum+1)) ;
    vii prev(maxsum+1) , curr(maxsum+1) ; vii nullvec(maxsum+1);
    curr[0]=1 ; curr[fac[0]]=1 ;
    for(int e=1 ;e<n ;e++){
        prev=curr ;
        curr=nullvec ;
        for(int f=0 ;f<=maxsum ;f++){
            if(prev[f]==1) {
                if(f+fac[e]<=maxsum)curr[f+fac[e]]=1 ;
                curr[f]=1 ;
            }
        }
    }
    ll ans =0; 
    for(int e=0 ;e<=maxsum ;e++){
        if(curr[e])ans = max(ans, 1LL*e*(maxsum-e)) ;
    }
    // for(auto v: fac)cout<<v<<" "; cout<<endl ;
    // cout<<ans<<endl;
    return ans ;
}

ll ansfinder(vector<vector<int>> & list ,int indx , vector<int> &nums,int parent){
    ll ans =0 ;
    vector<int> fac ;
    int faci =0 ;
    for(auto child: list[indx]){
        if(child== parent) continue ;
        ans+= ansfinder(list, child,nums,indx) ;
        fac.push_back(nums[child]) ; faci += nums[child] ;
    }
    // cout<<ans<<" ";
    if(fac.size()!=0){
        ans+=minidiff(fac,faci) ; 
    }
    return ans ;
}
...
int main(){
    ...
    for(int r=0 ;r<t ;r++){
        ...
        vector<int> nums(n) ;
        nums[0]=dfs(list, 0 , nums,-1) ;
        // dfs will return the total number of number
        cout<<ansfinder(list,0,nums,-1) ;
    }
}